...
exports.startDelhiveryStatusCron = startDelhiveryStatusCron;
exports.stopDelhiveryStatusCron = stopDelhiveryStatusCron;
exports.pollDelhiveryStatuses = pollDelhiveryStatuses;
...
function mapDelhiveryStatus(delhiveryStatus) {
    ...
    if (lower.includes("cancel"))
        return "cancelled";
    if (lower.includes("manifest") || lower.includes("not picked"))
        return "created";
    return null;
}
/**
 ...
 */
async function pollDelhiveryStatuses() {
    ...
    const activeShipments = await shipment_model_1.Shipment.find({
    ...
    for (let i = 0; i < activeShipments.length; i += BATCH_SIZE) {
        ...
        const results = await Promise.allSettled(batch.map(async (shipment) => {
            try {
                const response = await axios_1.default.get(`${delhiveryUrl}/api/v1/packages/json/?waybill=${shipment.waybill}`, {
                    headers: {
 ...
 */
function startDelhiveryStatusCron() {
...
function stopDelhiveryStatusCron() {
    if (cronInterval) {
