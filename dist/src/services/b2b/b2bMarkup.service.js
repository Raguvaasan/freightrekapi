...
exports.b2bMarkupService = void 0;
const mongoose_1 = require("mongoose");
const b2bMarkup_model_1 = require("../../models/b2b/b2bMarkup.model");
exports.b2bMarkupService = {
    /**
     * Get active B2B markup by category
     */
    async getMarkup(category) {
        try {
            const markup = await b2bMarkup_model_1.B2bMarkup.findOne({
    ...
    async createOrUpdateMarkup(category, data, currentUserId) {
        try {
            ...
            const currentUserObjId = new mongoose_1.Types.ObjectId(currentUserId);
            const existingMarkup = await b2bMarkup_model_1.B2bMarkup.findOne({
                markupCategory: category,
                isActive: true,
            });
            let result;
            ...
            if (existingMarkup) {
                ...
                await b2bMarkup_model_1.B2bMarkup.updateMany({ markupCategory: category, isActive: true }, { $set: { isActive: false } });
                const newMarkup = new b2bMarkup_model_1.B2bMarkup({
                    markupCategory: category,
                    markupType,
                    markupValue,
                    isActive: true,
     ...
     */
    async deleteMarkup(markupId, currentUserId) {
        try {
            const markup = await b2bMarkup_model_1.B2bMarkup.findOne({
                _id: new mongoose_1.Types.ObjectId(markupId),
                isActive: true,
            });
            if (!markup) {
                return {
            ...
            markup.updatedBy = new mongoose_1.Types.ObjectId(currentUserId);
            await markup.save();
            return {
                success: true,
