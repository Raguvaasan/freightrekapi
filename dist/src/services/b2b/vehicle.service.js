...
exports.b2bVehicleService = {
    ...
    async list(query) {
        ...
        const mongoQuery = {};
        if (search)
            mongoQuery.$or = [{ vehicleType: { $regex: search, $options: 'i' } }];
        if (status)
            mongoQuery.status = status;
        const [vehicles, total] = await Promise.all([
            b2bVehicle_model_1.B2bVehicle.find(mongoQuery).skip((page - 1) * limit).limit(limit).sort({ createdAt: -1 }),
            b2bVehicle_model_1.B2bVehicle.countDocuments(mongoQuery),
        ]);
    ...
    async getById(id) {
        ...
        const vehicle = await b2bVehicle_model_1.B2bVehicle.findById(id);
        if (!vehicle)
    ...
    async update(id, payload) {
        if (!mongoose_1.Types.ObjectId.isValid(id))
            return { success: false, message: 'Invalid vehicle ID' };
        const vehicle = await b2bVehicle_model_1.B2bVehicle.findByIdAndUpdate(id, {
    ...
    async deactivate(id) {
        if (!mongoose_1.Types.ObjectId.isValid(id))
            return { success: false, message: 'Invalid vehicle ID' };
        const vehicle = await b2bVehicle_model_1.B2bVehicle.findByIdAndUpdate(id, { status: 'Inactive' }, { new: true });
        if (!vehicle)
            return { success: false, message: 'Vehicle not found' };
    ...
    async delete(id) {
        if (!mongoose_1.Types.ObjectId.isValid(id))
            return { success: false, message: 'Invalid vehicle ID' };
        const vehicle = await b2bVehicle_model_1.B2bVehicle.findByIdAndDelete(id);
        if (!vehicle)
            return { success: false, message: 'Vehicle not found' };
