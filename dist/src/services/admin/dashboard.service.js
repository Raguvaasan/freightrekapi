...
class AdminDashboardService {
     ...
     */
    topAgenciesPipeline(periodMatch, limit) {
        return [
    ...
    async getTopAgencies(limit = 5, period = 'all') {
        try {
            const start = this.periodStart(period);
            const rows = await parcelOrder_model_1.ParcelOrder.aggregate(this.topAgenciesPipeline(start ? { createdAt: { $gte: start } } : {}, limit));
     ...
     */
    async getAdminDashboard(period = 'all') {
        try {
            ...
            const totalsFor = (match) => parcelOrder_model_1.ParcelOrder.aggregate([
                { $match: match },
                {
                    $group: {
                        _id: null,
            ...
            const [totalAgencies, activeAgencies, totalHubs, activeHubs, allTime, today, paymentTypeRows, topAgencyRows, recentOrders, walletSummary,] = await Promise.all([
                agency_model_1.Agency.countDocuments(),
                agency_model_1.Agency.countDocuments({ status: 'Active' }),
                hub_model_1.HubModel.countDocuments(),
                ...
                parcelOrder_model_1.ParcelOrder.aggregate(this.topAgenciesPipeline(periodMatch, 5)),
                // 8. Recent bookings
                parcelOrder_model_1.ParcelOrder.find()
                    .select('orderNumber agency bookingCustomer.name deliveryCustomer.name ' +
            ...
            const byType = new Map(paymentTypeRows.map((row) => [row._id, row]));
            const chartTotal = paymentTypeRows.reduce((sum, r) => sum + r.count, 0);
            const paymentTypeDistribution = parcelOrder_model_1.PAYMENT_TYPES.map((type) => {
                ...
                return {
                    ...
                    count,
                    amount: (0, walletLedger_1.round2)(row?.amount || 0),
                    percentage: chartTotal ? (0, walletLedger_1.round2)((count / chartTotal) * 100) : 0,
                };
            });
            return {
                success: true,
                data: {
                    overview: {
                        ...
                        totalRevenue: (0, walletLedger_1.round2)(totals.revenue),
                        todayOrders: todayTotals.orders,
                        todayRevenue: (0, walletLedger_1.round2)(todayTotals.revenue),
                        currency: '₹',
                    },
                    ...
                    recentBookings: recentOrders.map((order) => ({
                        ...
                        deliveryAgencyName: order.deliveryCustomer?.deliveryAgency?.agencyName || 'Unknown',
                        bookingCustomer: order.bookingCustomer?.name,
                        deliveryCustomer: order.deliveryCustomer?.name,
                        paymentType: order.paymentType,
                        paymentTypeLabel: PAYMENT_TYPE_LABELS[order.paymentType] || order.paymentType,
                        amount: (0, walletLedger_1.round2)(order.totalAmount || 0),
                        status: order.status,
    ...
    async getShipmentDashboard(period = 'week') {
        try {
            const now = new Date();
        ...
        try {
            ...
            return {
                ...
                data: {
                    ...
                    paymentForTruecargo: (0, walletLedger_1.round2)(split.adminShare),
                    /** Commission the agencies kept on those bookings */
                    agencyPayment: (0, walletLedger_1.round2)(split.agencyProfit),
                    settledOrders: split.settledOrders,
                    totalBookingAmount: (0, walletLedger_1.round2)(split.bookingAmount),
                    totalBalance: (0, walletLedger_1.round2)(wallets.totalBalance),
                    totalWallets: wallets.totalWallets,
                    credits: { amount: (0, walletLedger_1.round2)(txns.creditAmount), count: txns.creditCount },
                    debits: { amount: (0, walletLedger_1.round2)(txns.debitAmount), count: txns.debitCount },
                    currency: '₹',
                },
            };
        }
