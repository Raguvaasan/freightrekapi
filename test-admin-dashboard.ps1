...
try {
    ...
    if ($loginResponse.success) {
        ...
        Write-Host "  Admin: $($loginResponse.data.user.email)" -ForegroundColor Gray
...
try {
    ...
    if ($dashboardResponse.success) {
        ...
        Write-Host "✗ Dashboard API failed: $($dashboardResponse.message)" -ForegroundColor Red
    }
} catch {
...
try {
    ...
    if ($franchiseResponse.success) {
        Write-Host "✓ Top Franchises API working!" -ForegroundColor Green
        ...
        Write-Host "✗ Top Franchises API failed: $($franchiseResponse.message)" -ForegroundColor Red
    }
} catch {
    Write-Host "✗ Top Franchises API error: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    if ($walletResponse.success) {
        Write-Host "✓ Wallet Statistics API working!" -ForegroundColor Green
        ...
        Write-Host "  Credits: ₹$creditsAmount | Count: $creditsCount" -ForegroundColor Gray
        Write-Host "  Debits: ₹$debitsAmount | Count: $debitsCount" -ForegroundColor Gray
    } else {
        Write-Host "✗ Wallet Statistics API failed: $($walletResponse.message)" -ForegroundColor Red
    }
} catch {
    Write-Host "✗ Wallet Statistics API error: $($_.Exception.Message)" -ForegroundColor Red
}
