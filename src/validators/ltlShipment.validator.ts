...
const billingAddressSchema = yup.object().shape({
  ...
  'pan-or-gst',
  'Either PAN number or GST number is required',
  (value) => !!(value?.pan_number || value?.gst_number)
);

export const createLtlShipmentSchema = yup.object().shape({
...
export const getLtlShipmentsSchema = yup.object().shape({
...
export const updateLtlShipmentSchema = yup.object().shape({
