...
const bookingBody = {
  ...
  paymentType: yup
    ...
    .oneOf(PAYMENT_TYPES as unknown as string[], `Payment type must be one of: ${PAYMENT_TYPES.join(', ')}`),
...
export const createParcelOrderSchema = yup.object({
  body: yup
    .object({
    ...
    .test(
      'agency-required',
      'Agency is required',
      (value: any) => !!(value?.agency || value?.branch)
    ),
});

// Branch create: the branch comes from the logged-in franchise token
export const branchCreateParcelOrderSchema = yup.object({
  body: yup.object({
    ...bookingBody,
  }),
});
...
export const updateParcelOrderSchema = yup.object({
  body: yup.object({
    agency: yup.string().matches(objectId, 'Invalid agency ID').optional(),
    branch: yup.string().matches(objectId, 'Invalid agency ID').optional(),
    bookingCustomer: yup
...
export const assignHubSchema = yup.object({
  body: yup.object({
    hub: yup
      .string()
...
export const assignVehicleDriverSchema = yup.object({
  body: yup
    .object({
      vehicle: yup
        .string()
        ...
        .test(
          'vehicle-objectid',
          'Invalid vehicle ID',
          (value) => value === undefined || value === null || value === '' || objectId.test(value)
        ),
      driver: yup
        .string()
        .nullable()
    ...
    .test(
      'vehicle-or-driver',
      'Provide a vehicle and/or a driver',
      (body) => body?.vehicle !== undefined || body?.driver !== undefined
    ),
  params: orderIdParam,
});

// Update transportation charge validation schema
export const updateChargeSchema = yup.object({
  body: yup.object({
    transportationCharge: yup
      .number()
      .typeError('Transportation charge must be a number')
...
export const updateParcelStatusSchema = yup.object({
  body: yup.object({
    status: yup
      .string()
      .required('Status is required')
      .oneOf(PARCEL_STATUSES as unknown as string[], `Status must be one of: ${PARCEL_STATUSES.join(', ')}`),
    note: yup.string().max(500).trim().optional(),
  }),
  params: orderIdParam,
});
...
export const branchStatusSchema = yup.object({
  body: yup.object({
    status: yup
      .string()
      .required('Status is required')
      .oneOf(AGENCY_ALLOWED_STATUSES, `An agency can set only: ${AGENCY_ALLOWED_STATUSES.join(', ')}`),
    note: yup.string().max(500).trim().optional(),
  }),
  params: orderIdParam,
});
...
export const hubStatusSchema = yup.object({
  body: yup.object({
    status: yup
      .string()
      .required('Status is required')
      .oneOf(HUB_ALLOWED_STATUSES, `A hub can set only: ${HUB_ALLOWED_STATUSES.join(', ')}`),
    note: yup.string().max(500).trim().optional(),
  }),
  params: orderIdParam,
});
...
export const parcelOrderByIdSchema = yup.object({
  params: orderIdParam,
});
